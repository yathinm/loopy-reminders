...
import { Animated, PanResponder, Pressable, StyleSheet, Text, useColorScheme, View } from 'react-native';
...
export function ReminderRow({ reminder, onToggle, onPress, onFlag, onDelete }: { reminder: Reminder; onToggle: () => void; onPress: () => void; onFlag?: () => void; onDelete?: () => void }) {
  ...
  return <View style={[styles.container, { borderBottomColor: colors.border, backgroundColor: colors.surface }]} {...panResponder.panHandlers}>
    <View style={styles.actions}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Delete reminder" onPress={() => { close(); onDelete?.(); }} style={[styles.action, { backgroundColor: colors.danger }]}><Ionicons name="trash" size={23} color={colors.onColor} /><Text style={{ color: colors.onColor }}>Delete</Text></Pressable>
    </View>
...
const styles = StyleSheet.create({
  container: { minHeight: 72, borderBottomWidth: StyleSheet.hairlineWidth, overflow: 'hidden' }, content: { flex: 1, flexDirection: 'row', paddingVertical: 13, gap: spacing.md, zIndex: 1 },
  actions: { position: 'absolute', right: 0, top: 0, bottom: 0, width: 210, flexDirection: 'row' }, action: { width: 70, alignItems: 'center', justifyContent: 'center', gap: 3 },
