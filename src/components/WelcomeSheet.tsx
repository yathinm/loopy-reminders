...
export function WelcomeSheet({ visible, onFinish }: { visible: boolean; onFinish: () => Promise<void> }) {
  ...
  return <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.art, { backgroundColor: colors.softBrand }]}><View style={[styles.brandOrb, { backgroundColor: colors.brand }]}><View style={[styles.orbCenter, { backgroundColor: colors.accent }]} /></View></View>
...
const styles = StyleSheet.create({ container: { flex: 1, padding: 28, justifyContent: 'center' }, art: { height: 250, borderRadius: 36, alignItems: 'center', justifyContent: 'center' }, brandOrb: { width: 116, height: 116, borderRadius: 58, alignItems: 'center', justifyContent: 'center' }, orbCenter: { width: 38, height: 38, borderRadius: 19 }, title: { fontSize: 31, textAlign: 'center', fontWeight: '900', marginTop: 30 }, message: { fontSize: 17, lineHeight: 25, textAlign: 'center', marginTop: 10 }, dots: { flexDirection: 'row', justifyContent: 'center', gap: 7, marginVertical: 28 }, dot: { width: 8, height: 8, borderRadius: 4 }, button: { height: 56, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }, buttonText: { color: 'white', fontSize: 17, fontWeight: '800' } });
