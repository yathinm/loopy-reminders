...
export function ReminderEditor({ reminderId, initialListId }: { reminderId?: string; initialListId?: string }) {
  ...
  async function save() {
    const draft: ReminderDraft = { title, notes, dueAt: hasDate ? dueAt : null, hasTime: hasDate && hasTime, priority, isFlagged: flagged, listId, recurrence: hasDate ? recurrence : null, tagNames: reminder?.tags.map((tag) => tag.name) ?? [] };
  ...
  return (
    <KeyboardAvoidingView style={[styles.flex, { backgroundColor: colors.background }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.content}>
        ...
        <View style={[styles.mascotBanner, { backgroundColor: colors.softBrand }]}><LoopyMascot variant="angel" size={86} /><View style={styles.mascotCopy}><Text style={[styles.mascotTitle, { color: colors.text }]}>{reminderId ? 'Fine-tune your reminder' : 'A gentle nudge, right on time'}</Text><Text style={[styles.mascotMessage, { color: colors.secondaryText }]}>Loopy will help you remember.</Text></View></View>
