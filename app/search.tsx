...
export default function SearchScreen() {
  const router = useRouter(); const colors = colorsFor(useColorScheme()); const { reminders, toggleReminder, toggleFlag, deleteReminder } = useReminders();
  ...
  return <Screen>
    ...
    {!query.trim() ? <EmptyState title="Find anything" message="Search titles, notes, and tags." /> : results.length === 0 ? <EmptyState title="No matches" message="Loopy looked everywhere. Try another word." /> : <View style={[styles.list, { backgroundColor: colors.surface, borderColor: colors.border }]}>{results.map((item) => <ReminderRow key={item.id} reminder={item} onToggle={() => void toggleReminder(item.id)} onFlag={() => void toggleFlag(item.id)} onDelete={() => void deleteReminder(item.id)} onPress={() => router.push(`/reminder/${item.id}`)} />)}</View>}
  </Screen>;
}
const styles = StyleSheet.create({ search: { height: 48, borderRadius: 14, borderWidth: StyleSheet.hairlineWidth, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 13, gap: 9, marginBottom: 13 }, input: { flex: 1, fontSize: 16 }, list: { borderWidth: StyleSheet.hairlineWidth, borderRadius: 17, overflow: 'hidden' } });
