...
const listColors = [palette.dustyRose, palette.softPink, palette.blushPink, palette.deepPlum, palette.warmCoral, palette.softGray];
...
export default function ListEditorScreen() {
  ...
  const [color, setColor] = useState(existing?.color ?? listColors[0]!);
  ...
  return <Screen>
    ...
    <View style={[styles.preview, { backgroundColor: colors.softBrand }]}><LoopyMascot variant="angel" size={112} /><View style={styles.previewDetails}><View style={[styles.previewIcon, { backgroundColor: color }]}><Ionicons name={symbol} size={32} color={colors.onColor} /></View><Text style={[styles.previewText, { color: colors.text }]}>{name.trim() || 'New List'}</Text></View></View>
    ...
    <Text style={[styles.label, { color: colors.text }]}>Color</Text><View style={styles.choices}>{listColors.map((value) => <Pressable accessibilityRole="radio" accessibilityState={{ selected: color === value }} accessibilityLabel={`Choose color ${value}`} key={value} onPress={() => setColor(value)} style={[styles.color, { backgroundColor: value }, color === value && [styles.selected, { borderColor: colors.onColor }]]}>{color === value && <Ionicons name="checkmark" size={21} color={colors.onColor} />}</Pressable>)}</View>
...
const styles = StyleSheet.create({ actions: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 18 }, action: { fontSize: 17, fontWeight: '700' }, preview: { height: 150, borderRadius: 24, paddingHorizontal: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 24, marginBottom: 16, overflow: 'hidden' }, previewDetails: { alignItems: 'center' }, previewIcon: { width: 62, height: 62, borderRadius: 19, alignItems: 'center', justifyContent: 'center' }, previewText: { fontSize: 22, fontWeight: '800', marginTop: 9 }, input: { borderWidth: StyleSheet.hairlineWidth, borderRadius: 14, padding: 14, fontSize: 17 }, label: { fontSize: 17, fontWeight: '800', marginTop: 24, marginBottom: 12 }, choices: { flexDirection: 'row', flexWrap: 'wrap', gap: 13 }, color: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' }, selected: { borderWidth: 3 }, symbol: { width: 50, height: 50, borderRadius: 15, borderWidth: 1, alignItems: 'center', justifyContent: 'center' } });
