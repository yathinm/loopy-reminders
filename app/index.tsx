...
const smartLists: { id: SmartList; title: string; icon: keyof typeof Ionicons.glyphMap; color: string }[] = [
  { id: 'today', title: 'Today', icon: 'calendar', color: '#4E7AC7' },
  { id: 'scheduled', title: 'Scheduled', icon: 'time', color: '#A456A8' },
  { id: 'all', title: 'All', icon: 'file-tray-full', color: '#5D6774' },
  { id: 'flagged', title: 'Flagged', icon: 'flag', color: '#D37A2C' },
  { id: 'completed', title: 'Completed', icon: 'checkmark-circle', color: '#3B8A63' },
];
  ...
  return (
    <Screen>
      ...
      <View style={[styles.hero, { backgroundColor: colors.softBrand }]}>
        ...
        <View style={[styles.heroMark, { backgroundColor: colors.brand }]}><View style={[styles.heroMarkCenter, { backgroundColor: colors.accent }]} /></View>
      </View>
      ...
      <View style={styles.smartGrid}>
        {smartLists.map((item) => (
          <Pressable key={item.id} accessibilityRole="button" onPress={() => router.push({ pathname: '/list/[id]', params: { id: item.id, smart: '1' } })} style={({ pressed }) => [styles.smartCard, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.72 : 1 }]}>
            <View style={[styles.iconCircle, { backgroundColor: item.color }]}><Ionicons name={item.icon} size={19} color="white" /></View>
      ...
      <View style={[styles.listBox, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        {lists.map((list, index) => (
          <Pressable key={list.id} onPress={() => router.push({ pathname: '/list/[id]', params: { id: list.id } })} style={[styles.listRow, index < lists.length - 1 && { borderBottomColor: colors.border, borderBottomWidth: StyleSheet.hairlineWidth }]}>
            <View style={[styles.listIcon, { backgroundColor: list.color }]}><Ionicons name={list.symbol as keyof typeof Ionicons.glyphMap} size={18} color="white" /></View>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Add reminder" onPress={() => router.push('/reminder/new')} style={({ pressed }) => [styles.addButton, { backgroundColor: colors.accent, opacity: pressed ? 0.8 : 1 }]}><Ionicons name="add" size={24} color="white" /><Text style={styles.addText}>New Reminder</Text></Pressable>
    </Screen>
  );
}

const styles = StyleSheet.create({
  center: { flex: 1, alignItems: 'center', justifyContent: 'center' }, hero: { borderRadius: 24, padding: 18, flexDirection: 'row', alignItems: 'center', minHeight: 126 }, heroCopy: { flex: 1 }, heroMark: { width: 68, height: 68, borderRadius: 34, alignItems: 'center', justifyContent: 'center', marginLeft: 12 }, heroMarkCenter: { width: 22, height: 22, borderRadius: 11 }, greeting: { fontSize: 25, fontWeight: '900' }, subheading: { fontSize: 15, lineHeight: 21, marginTop: 6 }, error: { padding: 12, borderRadius: 12, marginTop: 12 },
